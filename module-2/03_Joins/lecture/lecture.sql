...
SELECT park_name, state_name, city_name AS capital, park.area AS park_area
FROM park
JOIN park_state ON park.park_id = park_state.park_id
JOIN state ON park_state.state_abbreviation = state.state_abbreviation
JOIN city ON state.capital = city.city_id;
...
SELECT state_name, COUNT(city_name) AS cities_in_midwest
FROM city
JOIN state ON city.state_abbreviation = state.state_abbreviation
WHERE census_region = 'Midwest'
GROUP BY state_name;
...
SELECT state_name, COUNT(city_name) AS num_city
FROM city
JOIN state ON city.state_abbreviation = state.state_abbreviation
WHERE census_region = 'Midwest'
GROUP BY state_name
...
SELECT state.state_name, MIN(park.date_established) AS date_park_established
FROM state
